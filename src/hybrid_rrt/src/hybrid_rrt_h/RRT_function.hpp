...
yaw_ability Check_Reachability(Node* q1, Node* q2, int flip_time){
    ...
    if (abs(wrap2pi(q2->yaw-steer)) > max_yaw_range ) range_flag3 = false;
...
bool is_incollsion(Node* node){
    for(size_t i = 0; i < obstacles.size(); ++i){
        ...
        if (Cal_Dist_2_obstacle(node, obstacles[i]) < 1.65*car_radius) {
                    cout << node->x <<" " << node->y << obstacles[i][0] << " " <<obstacles[i][1] << endl;
...
bool is_reach_goal(Node* node1, Node* node_goal){
    if(Cal_Dist(node1, node_goal) < threshod){
        if(abs(wrap2pi(node_goal->yaw- node1->yaw)) < 4*max_yaw_range){
...
Extend_return EXTEND(Tree &T, Node* q_rand, int* cout_back_off, int flip_time, int is_extend){
    ...
    if(*cout_back_off < max_attemp_num && q_new->direction == reverse_sign){ //cf true
