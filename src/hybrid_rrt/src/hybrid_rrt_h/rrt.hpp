...
vector<vector<double>> execute_rrt(vector<array<double, 2>> obstacles_hk, array<double, 5> start_hk, array<double, 5> end_hk){
    ...
    obstacles.clear();
    double goal2start_angle = atan2(q_goal.y-q_int.y, q_goal.x-q_int.x);

    if(fabs(goal2start_angle-q_int.yaw) > PI*0.5) reverse_sign = true;
    cout << "revrese is:  " << reverse_sign << endl;
...
//     }
    ...
    while(cur_node2 != NULL){
        ...
        //cout << cur_node2->x << " " << cur_node2->y << " " << cur_node2->yaw << " " << dir2 <<endl;
    ...
    for(int i = 0; i < final_path_output_reverse.size(); ++i){
        ...
        cout << final_path_output_reverse[i][0] << " " << final_path_output_reverse[i][1] << " " <<final_path_output_reverse[i][2] << " " << dir3 << endl; 
